...
func TestInstallChartAsync(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.releaseName, func(t *testing.T) {
			...
			var err error
			go func() {
				rel, err = InstallChartAsync(tt.namespace, tt.releaseName, tt.chartPath, nil, actionConfig, client, coreClient, true, tt.indexEntry)
				fmt.Println(rel, err)
				if tt.releaseName == "myrelease" {
					require.NoError(t, err)
					require.Equal(t, fmt.Sprintf("sh.helm.release.v1.%v.v1", tt.releaseName), rel.Name)
				} else if tt.releaseName == "invalid chart path" {
					require.Error(t, err)
				}
			}()
...
func TestInstallChartWithTlsDataAsync(t *testing.T) {
	//create the server.key and server.crt
	tests := []struct {
		releaseName     string
		chartPath       string
...
func TestInstallChartBasicAuthAsync(t *testing.T) {
	//create the server.key and server.crt
	tests := []struct {
		releaseName     string
		chartPath       string
